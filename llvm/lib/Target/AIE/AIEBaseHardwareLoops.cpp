...
void AIEBaseHardwareLoops::expandLoopStart(LowOverheadLoop &LoLoop) {
  ...
  if (LoLoop.isZeroOverheadLoop()) {
    ...
    BuildMI(*MBB, Start, Start->getDebugLoc(), TII->get(AIE2::MOVXM_lng_cg),
            AIE2::LS)
        .addMBB(LoLoop.LoopEnd->getOperand(0).getMBB());
    MachineInstrBuilder MIB;
    MIB = BuildMI(*MBB, Start, Start->getDebugLoc(),
                  TII->get(AIE2::MOVXM_lng_cg), AIE2::LE)
              .addSym(LoLoop.LoopEnd->getOperand(1).getMCSymbol());
    MachineInstr *MI = MIB.getInstr();
    MI->getOperand(1).ChangeToMCSymbol(
        LoLoop.LoopEnd->getOperand(1).getMCSymbol());
  }
